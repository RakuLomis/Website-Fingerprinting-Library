...
def summarize(cfg, datasets, models):
    rows = []
    results_root = Path(cfg["results_root"])
    for dataset in datasets:
        preprocess = maybe_read(dataset_dir(cfg, dataset) / "preprocess_summary.json")
        for model_name in models:
            metrics = maybe_read(results_root / "metrics" / dataset / model_name / "_metrics.json")
            profile = maybe_read(results_root / "profiles" / dataset / model_name / "_profile.json")
            latency = maybe_read(results_root / "latency" / dataset / model_name / "_cpu_latency.json")
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Summarize all WFlib fair-comparison outputs.")
    ...
    rows = summarize(cfg, datasets, models)
    for row in rows:
        print(row)
    return 0
